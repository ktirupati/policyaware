...
demo_app = typer.Typer(help="Zero-config demos for first-time users")
...
DEMO_DOCTOR_POLICY_TEMPLATE = """# PolicyAware demo policy
# Generated by: policyaware demo doctor
# Purpose: show policy doctor/checklist/normalize without external services.

id: policyaware_demo_doctor
schema_version: "0.3"
default: deny
rules:
  - name: block_secrets
  ...
  - name: require_approval_for_mcp_write_tools
    effect: require_approval
    when:
      request.action_type: tool_call
      request.tool_command_in:
        - write_file
        - execute_query
        - create_pr

  - name: enforce_agent_budget
    effect: deny
    when:
      request.max_tokens_gte: 4000

  - name: allow_low_risk_developers
    effect: allow
    when:
      user.role_in:
        - developer
...
def _policy_doctor_report(policy: dict[str, object]) -> dict[str, object]:
    lint_report = _policy_lint(policy)
...
def _render_policy_doctor_report(policy_file: Path, report: dict[str, object]) -> None:
    lint = report["lint"] if isinstance(report["lint"], dict) else {}
    findings = lint.get("findings", []) if isinstance(lint, dict) else []
    lint_counts = lint.get("counts", {}) if isinstance(lint, dict) else {}
    readiness = report["readiness"] if isinstance(report["readiness"], dict) else {}
    ...
    if isinstance(lint_counts, dict):
        table.add_row(
            "lint",
            "pass" if lint_counts == {"high": 0, "medium": 0, "low": 0} else "review",
            (
                f"high={lint_counts.get('high', 0)}, "
                f"medium={lint_counts.get('medium', 0)}, low={lint_counts.get('low', 0)}"
            ),
        )
...
def doctor_policy(
    ...
    policy = _load_yaml_mapping(policy_file)
    report = _policy_doctor_report(policy)
    if json_output:
        console.print_json(data=report)
    else:
        _render_policy_doctor_report(policy_file, report)
    readiness = report["readiness"] if isinstance(report["readiness"], dict) else {}
    severity_counts = _checklist_severity_counts(readiness)
    if _should_fail_scan(fail_on, severity_counts):
        raise typer.Exit(code=1)
...
@demo_app.command("doctor")
def demo_doctor(
    out: Path = typer.Option(
        Path(".policyaware/demo/policyaware.demo.yaml"),
        "--out",
        "-o",
        help="Demo policy file to write before running policy doctor.",
    ),
    json_output: bool = typer.Option(False, "--json", help="Print doctor report as JSON after writing the demo policy."),
    force: bool = typer.Option(False, "--force", help="Overwrite the demo policy file if it exists."),
) -> None:
    ...
    report = _policy_doctor_report(policy)
    if json_output:
        console.print_json(data={"policy_file": str(out), **report})
        return
    console.print(f"[bold green]Created demo policy:[/bold green] {out}")
    _render_policy_doctor_report(out, report)
    console.print("Try next:")
    console.print(f"  policyaware policy checklist {out}")
    console.print(f"  policyaware policy normalize {out} --out {out.with_suffix('.normalized.yaml')}")
